# Todo List

## 优先级 1：协同与权限系统 (Collaboration & Auth)

- [x] **标题协同 (Y.js 托管)**：将笔记标题纳入 Y.js 管理，解决弱网下的乱序与冲突问题。
- [x] **分享链接与鉴权**：生成分享链接，验证访问权限。
- [x] **权限管理面板**：管理笔记属性与用户权限。
- [x] **细粒度权限控制 (RBAC)**：
  - [x] Owner：完全控制（分享、修改所有权限）。
  - [x] Admin：可分享、修改读写权限。
  - [x] Editor：仅编辑。
  - [x] Viewer：仅查看。
- [x] **实时协作者统计**：在标题旁显示当前在线用户数/头像。

## 优先级 2：编辑器体验优化 (Editor UX)

- [x] **滚动修复**：解决编辑区域无法滚动的问题。
- [x] **布局优化**：
  - [x] 底部留白 (Padding bottom)。
  - [x] 边框与容器调整 (去除两侧奇怪间距，优化居中体验)。
- [x] **Slash 命令菜单**：实现 `/` 唤起快捷菜单，替代/隐藏顶部固定工具栏。
- [x] **快捷键拦截**：拦截 `Ctrl+S` 触发应用内保存。
- [x] **大纲/目录 (TOC)**：基于标题自动生成，支持折叠。

## 优先级 3：导航与组织 (Navigation & Organization)

- [x] **笔记列表排序**：支持多种排序方式。
- [x] **快速添加笔记**：基于搜索页面的快速归档/添加功能。
- [x] **列表交互优化**：
  - [x] 列表可折叠。
  - [x] 移动端适配 (Mobile responsive)。
- [x] **全局搜索**：全局搜索页面与组件。

## 优先级 4：高级功能与性能 (Advanced & Performance)

- [ ] **历史记录**：版本回溯功能。
- [ ] **离线与预加载**：
  - [ ] 利用 IndexedDB 缓存笔记标题、知识库属性。
  - [ ] 弱网环境优化。
- [ ] **PWA (渐进式 Web 应用)**：配置 Manifest，Service Worker 离线支持。

## 已完成 (Completed)

- [x] 报错：`legacyBehavior` is deprecated (Next.js Link)
- [x] 知识库无法删除
- [x] 编辑器样式优化 (Tiptap Typography & Theme)
- [x] 自动保存 (Auto-save)
- [x] 快捷键支持 (Hotkeys)
- [x] 个人页面 (Profile Page)
- [x] 知识库搜索功能
- [x] 知识库按日期排序功能
